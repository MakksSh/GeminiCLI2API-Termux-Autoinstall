...
step_70_run_app() {
  ...
  log "Дальше открой новую сессию Termux и запускай таверну там. Потяни слева экрана вправа, кнопка New Session."
  log "При первом запуске geminicli2api появится ссылка для авторизации."
  log "Скопируй ссылку и открой в браузере, авторизуйся с аккаунтом номер проекта от которого указал вначале."
