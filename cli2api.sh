...
step_70_run_app() {
  ...
  log "Важно: эту сессию оставь открытой. Остановить можно Ctrl+C."
  log "Дальше открой новую сессию Termux и запускай таверну там."
  log "При первом запуске появится ссылка для авторизации."
  log "Скопируй и открой в браузере, авторизуйся с аккаунтом номер которого указал вначале."
